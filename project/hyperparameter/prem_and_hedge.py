...
UPGRADE OVER THE ORIGINAL: the single scalar premium has been replaced by a small
premium network that maps the initial moneyness S0/K to a premium. Minimising MSE now
drives this network towards pi(S0) = E_Q[V_N | S0] = C_BS(S0), i.e. the moneyness-
dependent fair price, instead of a single average price charged to every option. The
practitioner BSM benchmark has been matched (per-path premium) so the comparison stays
fair.
...
PREMIUM_LR_SCALE = 0.5       # premium head trains at HP["lr"] * this
# -------------------
...
class HedgingNet(nn.Module):
    ...
    The premium is no longer a single scalar parameter. A small premium head maps the
    initial moneyness S0/K to a premium, so the model learns a fair price per option
    rather than one average price for the whole moneyness band.
    """
    def __init__(self, input_dim=4, hidden=128, depth=4):
        ...
        # premium head: S0/K -> premium. Final layer zero-initialised so the premium
        # starts at ~0 and is discovered from the gradient signal, not seeded with a price.
    ...
    def premium_for(self, S0):
        """Premium as a function of the initial price S0 (shape (batch,))."""
...
def practitioner_bsm_hedge_pnl(S, moneynesses):
    """Practitioner BSM: uses realised vol at each rebalancing date and a per-path premium.

    This is the fair benchmark for the relvol model -- same information as the network.
    The premium is now charged per option (BSM price at sigma=0.2), matching the
    network's premium head, so neither side gets a free pricing-dispersion advantage.
    """
    n_paths    = S.shape[1]
    currency   = np.zeros(n_paths)
    underlying = np.zeros(n_paths)
    ...
    # per-path premium: BSM price using sigma=0.2 as a reasonable a-priori estimate
...
def train(S_train):
    ...
    # separate, gentler learning rate for the premium head: its gradient scale differs
    # from the delta net, so it gets its own param group rather than a second optimiser.
    opt = optim.Adam([
...
def diagnose_premium_curve(net):
    """Compare the learned premium curve against the BS price across moneyness."""
    print("\n=== Premium curve vs BS price (sigma=0.2) ===")
    S0_grid = np.linspace(MONEYNESS_RANGE[0], MONEYNESS_RANGE[1], 7)
    with torch.no_grad():
        learned = net.premium_for(
            torch.tensor(S0_grid, dtype=torch.float32, device=DEVICE)
        ).cpu().numpy()
    bs_ref = bsm_call_vec(S0_grid, np.full_like(S0_grid, 0.2))
    print(f"  {'S0/K':>6}  {'learned':>9}  {'BS(0.2)':>9}  {'diff':>9}")
    for s0, lp, bp in zip(S0_grid, learned, bs_ref):
        print(f"  {s0:>6.3f}  {lp:>9.4f}  {bp:>9.4f}  {lp - bp:>+9.4f}")
...
def evaluate(S_test, sigmas, moneynesses, net):
    ...
    unhedged         = prem_test - np.maximum(S_test[DAILY_STEPS, :] - K, 0)
    ...
    plot_premium_curve(net)
    plot_delta_paths(S_test, sigmas, net)
    diagnose_premium_curve(net)
    diagnose_features(S_test, sigmas, net)
    diagnose_delta_by_sigma(S_test, sigmas, net)
    print(f"\nPlots saved with tag '{TAG}'.")


def plot_premium_curve(net):
    """Plot the learned premium against the BS price across the moneyness band (Fig 5.1)."""
    S0_grid = np.linspace(MONEYNESS_RANGE[0], MONEYNESS_RANGE[1], 100)
    with torch.no_grad():
        learned = net.premium_for(
            torch.tensor(S0_grid, dtype=torch.float32, device=DEVICE)
        ).cpu().numpy()
    ...
    ax.plot(S0_grid, learned, label="learned premium", linewidth=1.4)
    ax.plot(S0_grid, bs_ref, label="BS price ($\\sigma=0.2$)", linewidth=1.4, linestyle="--")
    ax.set_title(f"Learned premium vs BS price  ({TAG})")
    ax.set_xlabel("$S_0 / K$")
    ax.set_ylabel("premium")
    ax.legend(fontsize=9)
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, f"premium_{TAG}.png"), dpi=150, bbox_inches="tight")
    plt.close()
