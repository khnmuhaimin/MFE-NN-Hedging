...
def do_NN(MODEL: str):
    ...
    N_PATHS_TEST  = 10_000
    # BATCH_SIZE    = 2_048  # Set using model selection
    # N_EPOCHS      = 500  # Set using model selection
    # LEARNING_RATE = 1e-3  # Set using model selection
    ...
    class HedgingNet(nn.Module):
        ...
        def __init__(self, n_features: int, hidden: int, depth: int, gamma: float):
            ...
            # layers.append(nn.LeakyReLU(negative_slope=gamma))
    ...
    def plot_learning_curve(epoch_losses: list) -> None:
        ...
        plt.tight_layout()
        plt.savefig(project_path(f"results/figures/{MODEL}_learning_curve.png"), dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def plot_delta_paths(features_test: torch.Tensor, Sigma_test: np.ndarray, bsm_deltas, hedging_net: HedgingNet) -> None:
        ...
        plt.tight_layout()
        plt.savefig(project_path(f"results/figures/{MODEL}_delta_paths.png"), dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def test(features_test: torch.Tensor, Sigma_test: np.ndarray, bsm_deltas, hedging_net: HedgingNet) -> dict:
        ...
        plt.tight_layout()
        plt.savefig(project_path(f"results/figures/{MODEL}_pnl_distribution.png"), dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def save_results(epoch_losses: list, test_stats: dict) -> None:
        path = project_path(f"results/logs/{MODEL}_training_results.txt")
    ...
    def save_model(hedging_net: HedgingNet) -> None:
        path = project_path(f"results/models/{MODEL}_hedging_model.pt")
        torch.save({
...
do_NN("base_monthly")
do_NN("base_daily")
do_NN("base_relvol_bsdelta_monthly")
do_NN("base_relvol_bsdelta_daily")